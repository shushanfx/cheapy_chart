(function($){
    ...
    CheapyChart.prototype.initSeries = function(){
        ...
        if(!window.isNaN(options.yAxis.axisLine.maxValue)){
            this.maxValue = Math.max(this.maxValue, options.yAxis.axisLine.maxValue);
        }
        if(!window.isNaN(options.yAxis.axisLine.minValue)){
            this.minValue = Math.min(this.minValue, options.yAxis.axisLine.minValue);
        }
    };
    ...
    CheapyChart.prototype.renderSpecialLine = function(){
        // tooltip
        ...
        if(me.prevPosition){
            me._registerTick(function(){
                ...
                //$el.append($toolline);
                me._drawLine(me.prevPosition[0], y, me.prevPosition[0], h -  y2, tooltip.lineStyle);
            });
        }
    };
        ...
        var option = this.options,
            ...
            recursiveFunction = function(){
                ...
                for(var i= start; i < end; i++){
                    i < size && typeof(list[i]) === "function" && list[i]();
                }
                iCount ++;
                if(end >= size){
                    me._stopTick();
    ...
    CheapyChart.prototype.getPixFromEvent = function(event){
        //var canvas = this.canvas, touch = event;
    ...
    CheapyChart.prototype._showToolTip = function(pos, e){
        ...
        var isSamePosition = function(pos) {
            if(!me.prevPosition){
                me.prevPosition = pos;
            }
            else if(me.prevPosition[0] === pos[0] && me.prevPosition[1] === pos[1]) {
                return true;
            }
            me.prevPosition = pos;
            return false;
